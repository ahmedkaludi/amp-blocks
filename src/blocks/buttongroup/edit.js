...
class ampAdvancedButton extends Component {
	...
	componentDidMount() {
		if ( ! this.props.attributes.uniqueID ) {
			...
			ktadvancedbuttonUniqueIDs.push( '_' + this.props.clientId.substr( 2, 3 ) );
	...
	render() {
		...
		return (
			<Fragment>
				{ renderCSS }
				<div id={ `b_${ uniqueID }` } className={ `${ className } bgp b-align-${ hAlign }${ ( forceFullwidth ? ' amp-force-btn-fullwidth' : '' ) }` }>
					<BlockControls>
						<AlignmentToolbar
							value={ hAlign }
