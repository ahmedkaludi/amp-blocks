	...
	deprecated: [
		{
			...
			save: ( { attributes } ) => {
				...
				if ('ct' === bottomSep) {
					...
					bottomSVGDivider =
						<path d="M1000,100c0,0 -270.987,-98 -500,-98c-229.013,0 -500,98 -500,98l1000,0Z"/>;
					...
					bottomSVGDivider =
						<path d="M1000,100c0,0 -420.987,-98 -650,-98c-229.013,0 -350,98 -350,98l1000,0Z"/>;
					...
					bottomSVGDivider =
						<path d="M1000,100c0,0 -120.987,-98 -350,-98c-229.013,0 -650,98 -650,98l1000,0Z"/>;
				...
				const firstColumnClass = ( firstColumnWidth && columns === 2 ? ' amp-custom-first-width-' + firstColumnWidth : '' );
				...
				const innerClasses = classnames( `amp-row-layout-inner ${ hasBG } amp-layout-id${ classId }` );
				const innerColumnClasses = classnames( `amp-row-column-wrap amp-has-${ columns }-columns amp-gutter-${ columnGutter } amp-row-valign-${ verticalAlignment } amp-row-layout-${ layoutClass } amp-tab-layout-${ tabLayoutClass } amp-m-colapse-${ collapseOrder } amp-mobile-layout-${ mobileLayoutClass }${ firstColumnClass }` );

				return (
					<HtmlTagOut className={ classes }>
						<div id={ `amp-layout-id${ uniqueID }` } className={ innerClasses }>
							{ ( overlay || overlayBgImg ) && (
								<div className={ `amp-row-layout-overlay amp-row-overlay-${ overlayType }` }></div>
							) }
							{ topSep && 'none' !== topSep && '' !== topSep && (
								<div className={ `amp-row-layout-top-sep amp-row-sep-type-${ topSep }` }>
									<svg style={ { fill: topSepColor } } viewBox="0 0 1000 100"
							...
							{ bottomSep && 'none' !== bottomSep && '' !== bottomSep && (
								<div className={ `amp-row-layout-bottom-sep amp-row-sep-type-${ bottomSep }` }>
									<svg style={ { fill: bottomSepColor } } viewBox="0 0 1000 100"
										 preserveAspectRatio="none">
										{ bottomSVGDivider }
									</svg>
								</div>
		...
		},
		{
			...
			save: ( { attributes } ) => {
				...
				const classes = classnames( `align${ blockAlignment }` );
				const innerClasses = classnames( `amp-row-layout-inner ${ hasBG } amp-layout-id${ classId }` );
				const innerColumnClasses = classnames( `amp-row-column-wrap amp-has-${ columns }-columns amp-gutter-${ columnGutter } amp-row-valign-${ verticalAlignment } amp-row-layout-${ layoutClass } amp-tab-layout-${ tabLayoutClass } amp-m-colapse-${ collapseOrder } amp-mobile-layout-${ mobileLayoutClass }` );
